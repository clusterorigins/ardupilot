...
class MagTest
{
    friend class AP_MagTest_Backend;
    //UAVCAN drivers are initialised in the Backend, hence list of drivers is needed there.
    friend class AP_MagTest_UAVCAN;
public:
    MagTest();

    /* Do not allow copies */
    MagTest(const MagTest &other) = delete;
    MagTest &operator=(const MagTest&) = delete;
...
//        PX4    = 4, // no longer used, but may be in some user's parameters
    ...
    struct MagTest_State {
        uint16_t distance_cm;           // distance: in cm
        uint16_t voltage_mv;            // voltage in millivolts, if applicable, otherwise 0
        enum MagTest::Status status; // sensor status
    ...
    */
    ...
    AP_MagTest_Backend *find_instance(enum Rotation orientation) const;

    AP_MagTest_Backend *get_backend(uint8_t id) const;
    ...
    MagTest::Status status_orient(enum Rotation orientation) const;
    ...
    static MagTest *get_singleton(void) { return _singleton; }

protected:
    AP_MagTest_Params params[RANGEFINDER_MAX_INSTANCES];

private:
    static MagTest *_singleton;

    MagTest_State state[RANGEFINDER_MAX_INSTANCES];
    AP_MagTest_Backend *drivers[RANGEFINDER_MAX_INSTANCES];
    ...
    bool _add_backend(AP_MagTest_Backend *driver, uint8_t instance);
